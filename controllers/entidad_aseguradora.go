...
func (c *EntidadAseguradoraController) GetOne() {
	...
	v, err := models.GetEntidadAseguradoraById(id)
	if err != nil {
		logs.Error(err)
		//c.Data["development"] = map[string]interface{}{"Code": "000", "Body": err.Error(), "Type": "error"}
		c.Data["system"] = err
...
func (c *EntidadAseguradoraController) GetAll() {
	...
	l, err := models.GetAllEntidadAseguradora(query, fields, sortby, order, offset, limit)
	if err != nil {
		logs.Error(err)
		//c.Data["development"] = map[string]interface{}{"Code": "000", "Body": err.Error(), "Type": "error"}
		c.Data["system"] = err
...
// @Param	body		body 	models.EntidadAseguradora	true		"body for EntidadAseguradora content"
// @Success 200 {object} models.EntidadAseguradora
// @Failure 400 the request contains incorrect syntaxt
// @router /:id [put]
func (c *EntidadAseguradoraController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v := models.EntidadAseguradora{Id: id}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if err := models.UpdateEntidadAseguradoraById(&v); err == nil {
			c.Data["json"] = v
		} else {
			logs.Error(err)
			//c.Data["development"] = map[string]interface{}{"Code": "000", "Body": err.Error(), "Type": "error"}
...
func (c *EntidadAseguradoraController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	if err := models.DeleteEntidadAseguradora(id); err == nil {
